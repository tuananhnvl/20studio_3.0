...
scroller.on(event => {
  posYF = (-event.y * 0.0002)
   localStorage.setItem('posY',posYF)
})
const loaderAnim = new GLTFLoader();

let scene, renderer, camera, ModelBlender,animMixerModel
...
loaderAnim.load('./asset/animFF.glb', function (gltf) {
    ModelBlender = gltf;
    animMixerModel = new THREE.AnimationMixer(ModelBlender.scene);
     animMixerModel.clipAction(ModelBlender.animations[0]).play()
   /* 

  
    ModelBlender.scene.children[0].material.side = 2
    console.log(ModelBlender.scene.children[0].material.side,ModelBlender.animations[0].tracks[0].values
        ) */
    console.log(ModelBlender.animations[0])
    let modelOut =  ModelBlender.scene
    scene.add(modelOut);
...
let posY = localStorage.getItem('posY')

  if(ModelBlender) {
 
  
 
  //  animMixerModel.update(t/500.);
    ModelBlender.scene.rotation.x  = 2
